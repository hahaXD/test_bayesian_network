...
def generate_distribution(num_states,alpha=0.5,beta=1.0, cpt_selection="gamma"):
    if cpt_selection == "random":
        print ("Generating using random")
        pr = [random.random() for i in range(num_states)]
    else:
        print ("Generating using gamma")
        pr = [ random.gammavariate(alpha,beta) for i in range(num_states) ]
    pr_sum = sum(pr)
    print ([ p/pr_sum for p in pr ])
    return [ p/pr_sum for p in pr ]

def SimulateHmm(hmm_config, num_examples, cpt_selection="gamma"):
    window_size = hmm_config["window_size"]
    chain_size = hmm_config["chain_size"]
    hidden_state_size = hmm_config["hidden_state_size"]
    parameter_seed = hmm_config["parameter_seed"]
    simulate_seed = hmm_config["simulate_seed"]
    random.seed(parameter_seed)
    # simulate parameters
    factor_size = int(math.pow(hidden_state_size, window_size))
    prior_hidden_cpt = generate_distribution(factor_size, cpt_selection=cpt_selection)
    conditional_cpt_factor_size = factor_size * hidden_state_size
    conditional_cpt = []
    for i in range(0, factor_size):
    ...
    emission_cpt = [0] * hidden_state_size
    ...
    for i in range(0, num_examples):
        ...
        h_start_conf = decode_index(simulated_h_start, hidden_state_size, window_size)
        simulated_e = []
        simulated_h = list(h_start_conf)
        for j in range(window_size, chain_size):
            parent_conf = simulated_h[-window_size :]
...
def inference(prior_cpt, conditional_cpt, emission_cpt, observation, hidden_state_size, chain_size, window_size):
    ...
    for i in range(0, len(prior_cpt)):
        cur_conf = decode_index(i, hidden_state_size, window_size)
        for j in range(0, window_size):
            message[i] *= h_softs[j][cur_conf[j]]
    # sum out h_s
    message_size = len(message)
    for i in range(window_size, chain_size):
        new_message = []
...
def GenerateRandomHardDatasetByMissingFromHidden(hmm_config):
    ...
    for cur_data in examples:
        ...
        pr = inference(prior_cpt, conditional_cpt, emission_cpt, evidence, hidden_state_size, chain_size, window_size)
        pr_evid = sum(pr)
...
def GenerateRandomHardDatasetWithMCAR(hmm_config, cpt_selection):
    num_examples = hmm_config ["num_examples"]
    hidden_state_size = hmm_config["hidden_state_size"]
    chain_size = hmm_config["chain_size"]
    simulate_seed = hmm_config["simulate_seed"]
    window_size = hmm_config["window_size"]
    missing_pr = 0.2 
    prior_cpt, conditional_cpt, emission_cpt, examples = SimulateHmm(hmm_config, num_examples, cpt_selection)
    missing_result = []
    random.seed(simulate_seed)
    for cur_data in examples:
        cur_record = {}
...
def GenerateRandomHardDataset(hmm_config):
    num_examples = hmm_config ["num_examples"]
    hidden_state_size = hmm_config["hidden_state_size"]
    chain_size = hmm_config["chain_size"]
    simulate_seed = hmm_config["simulate_seed"]
    ...
    return result, prior_cpt, conditional_cpt, emission_cpt
...
"""
hmm_config = {}
hmm_config["window_size"] = 2
hmm_config["chain_size"] = 10
...
observation = [0.5] * hmm_config["chain_size"]
observation[-1] = 1
inf_result = inference(prior, conditional, emission, observation, hmm_config["hidden_state_size"], hmm_config["chain_size"], hmm_config["window_size"])
