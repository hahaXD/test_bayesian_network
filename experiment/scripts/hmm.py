...
class HmmParameterGeneratorRandomTransition:
    ...
    def generate_transition_cpt(self):
        ...
        for i in range(0, num_parent_states):
            ...
            cur_dist =  self.generate_from_dirchlet([ 1 for j in range(0, self.cardinality)])
            cpt[i] = cur_dist
...
class HmmParameterGeneratorPartialDetTransition:
    def __init__(self, cardinality, window_size, emission_error, seed, det_pr):
        self.cardinality = cardinality
        self.window_size = window_size
        self.emission_error = emission_error
        self.random_generator = random.Random(seed)
    ...
    def generate_transition_cpt(self):
        ...
        for i in range(0, num_parent_states):
            #cur_parent_config = np.unravel_index(i, [self.cardinality] * self.window_size)
            if self.random_generator.random() <= self.det_pr:
                # spit a determinsintic transition
                det_state = int(self.random_generator.random() * self.cardinality) %  self.cardinality
                cpt[i] = [0 if a != det_state else 1 for a in range(0, self.cardinality)]
            else:
                cpt[i] = self.generate_from_dirchlet([1 for a in range(0, self.cardinality)])
        return cpt.reshape([self.cardinality] * (self.window_size+1))
...
class HmmParameterGenerateorWithPeak:
    def __init__(self, cardinality, window_size, emission_error, seed = None):
        self.cardinality = cardinality
        self.window_size = window_size
        self.emission_error = emission_error
        if seed is not None:
    ...
    def generate_transition_cpt(self, index = 0):
        print ("%s index " % index)
        num_parent_states = int(math.pow(self.cardinality, self.window_size))
        cpt = np.zeros([self.cardinality]*(self.window_size + 1)).reshape([num_parent_states, self.cardinality])
        offset = int(self.random_generator.random() * num_parent_states) if self.random_generator is not None else 0
        for i in range(0, num_parent_states):
            cur_peak_dist = self.generate_peak_distribution(float(i + offset) % num_parent_states/(num_parent_states-1))
            if (index % 2) == 1:
                cur_peak_dist = [a for a in reversed(cur_peak_dist)]
            #cur_peak_dist = self.generate_peak_distribution(float(i + offset) % num_parent_states/(num_parent_states-1))
            cpt[i] = cur_peak_dist
        return cpt.reshape([self.cardinality] * (self.window_size+1))
...
class HetHmm:
    def __init__(self, chain_size, window_size, cardinality, parameter_generator):
        ...
        for i in range(self.window_size, self.chain_size):
            self.transition_cpt[i] = parameter_generator.generate_transition_cpt(i)
    ...
    def generate_dataset_with_soft_evidence (self, num_examples, seed):
        r_generator = random.Random(seed)
        records = []
        for i in range(0, num_examples):
            cur_evid = [r_generator.random() for j in range(0, self.chain_size)]
            """
            for i in range(0, self.chain_size):
    ...
    def sample(self, sim_rand_gen):
        ...
        for j in range(self.window_size, self.chain_size):
            ...
            local_cpt = self.transition_cpt[j][parent_conf]
    ...
    def inference(self, evidence):
        ...
        for i in range(self.window_size, self.chain_size):
            ...
            for j in range(0, size_after_multiply):
                ...
                new_message[j] = message[cur_conf[:-1]] * self.transition_cpt[i][cur_conf] * cur_soft_evid
...
"""
generator = HmmParameterGeneratorRandomTransition(2, 1, 0)
hmm = HetHmm(8, 1, 2, generator)
rand_gen = random.Random(0)
q = {}
for i in range(0,10000):
    h_sampled, _ = hmm.sample(rand_gen)
    k = q.setdefault(h_sampled[-1], 0)
    q[h_sampled[-1]] = k+1
res = [float(q[a])/10000 for a in range(0, len(q))]
evid = [0.5] * 8
print (hmm.inference(evid))
print (res)
"""
